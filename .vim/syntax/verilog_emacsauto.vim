...
function s:Add()
   ...
   w! %.emacsautotmp
   silent !xemacs -batch -l ~/elisp/verilog-mode.el %.emacsautotmp -f verilog-batch-auto 
   silent % !cat %.emacsautotmp 
   "if &expandtab
      "retab
      "let &tabstop=s:save_tabstop
   "endif
   silent !\rm -f %.emacsautotmp
endfunction
...
function s:Delete()
   ...
   " maybe add /tmp to the temporary filename
   w! %.emacsautotmp
   silent !xemacs -batch -l ~/elisp/verilog-mode.el %.emacsautotmp -f verilog-batch-delete-auto
   silent % !cat %.emacsautotmp 
   silent !\rm -f %.emacsautotmp
endfunction
